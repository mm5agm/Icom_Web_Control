...
namespace Icom_Web_Control.Controllers
{
    ...
    public class VideoController : ControllerBase
    {
            ...
            try
            {
                ...
                else if (OperatingSystem.IsMacOS() && devices.Count == 0)
                {
                    notes =
                        ...
                        "then allow Camera under System Settings → Privacy & Security → Camera → Icom Web Control, and refresh. " +
        ...
        public async Task<IActionResult> Status()
        {
            ...
            var rates = VideoDeviceFpsCaps.RatesFor(s.DeviceKey, allowDeviceOpen: !_capture.IsCapturing);
            var targetFps = VideoFpsOptions.Normalize(s.TargetFps, rates);
            var sizes = VideoDeviceSizeCaps.SizesFor(s.DeviceKey, allowDeviceOpen: !_capture.IsCapturing);
            return Ok(new
            {
                enabled = s.DisplayEnabled,
                ...
                rates,
                captureSize = VideoSizeOptions.Normalize(s.CaptureSize, sizes),
                sizes,
                jpegQuality = VideoJpegQualityOptions.Normalize(s.JpegQuality)
            });
        }
        ...
        public async Task<IActionResult> SetDevice([FromBody] DeviceRequest? body)
        {
            ...
            return Ok(new { deviceKey = s.DeviceKey, status = _capture.Status, maxFps = VideoFpsOptions.Max(rates), rates });
        }
        ...
        public async Task<IActionResult> SetFps([FromBody] FpsRequest? body)
        {
            ...
            var rates = VideoDeviceFpsCaps.RatesFor(s.DeviceKey, allowDeviceOpen: !_capture.IsCapturing);
            s = s with { TargetFps = VideoFpsOptions.Normalize(body?.Fps ?? s.TargetFps, rates) };
            await _settings.SaveAsync(s);
            return Ok(new { targetFps = s.TargetFps, maxFps = VideoFpsOptions.Max(rates), rates });
        }
        ...
        public async Task<IActionResult> SetSize([FromBody] SizeRequest? body)
        {
            ...
            var changed = !string.Equals(s.CaptureSize ?? "", normalized, StringComparison.Ordinal);
            s = s with { CaptureSize = normalized };
            await _settings.SaveAsync(s);
        ...
        public async Task<IActionResult> SetJpegQuality([FromBody] JpegQualityRequest? body)
        {
            ...
            s = s with { JpegQuality = VideoJpegQualityOptions.Normalize(body?.Quality ?? s.JpegQuality) };
            await _settings.SaveAsync(s);
            return Ok(new { jpegQuality = s.JpegQuality });
        }
        ...
        public async Task<IActionResult> Start()
        {
            ...
            if (string.IsNullOrWhiteSpace(s.DeviceKey))
