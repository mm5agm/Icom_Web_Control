...
namespace Icom_Web_Control.Services.Video
{
    ...
    public sealed class VideoCaptureService : IDisposable
    {
        ...
        private readonly IVideoSettingsSource _settings;
        ...
        private void RunCaptureLoopCore(CancellationToken ct, int epoch)
        {
            ...
            while (!ct.IsCancellationRequested && Volatile.Read(ref _captureEpoch) == epoch)
            {
                ...
                if (!settings.DisplayEnabled ||
                    ...
                    !VideoDeviceKey.TryResolveOpenIndex(settings.DeviceKey, out var index))
                {
                ...
                var deviceKey = settings.DeviceKey;
                var captureSize = VideoSizeOptions.Normalize(
                    settings.CaptureSize, VideoDeviceSizeCaps.PeekSizes(deviceKey));
                var maxWidth = EffectiveMaxWidth(settings, captureSize);
                var rates = VideoDeviceFpsCaps.PeekRates(settings.DeviceKey);
                var targetFps = VideoFpsOptions.Normalize(settings.TargetFps, rates);
                var jpegQuality = VideoJpegQualityOptions.Normalize(settings.JpegQuality);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (settingsAge.ElapsedMilliseconds >= SettingsRefreshMs)
                        {
                            ...
                            var newFps = VideoFpsOptions.Normalize(settings.TargetFps, VideoDeviceFpsCaps.PeekRates(settings.DeviceKey));
                            jpegQuality = VideoJpegQualityOptions.Normalize(settings.JpegQuality);
                                ...
                                if (OperatingSystem.IsMacOS())
                                {
                                    ...
                                    MacAvFoundationCapture.OnUiThread(() =>
                                    {
                                        MacAvFoundationDevices.TrySetFrameRate(
                                            idx, UniqueIdFromDeviceKey(settings.DeviceKey),
                        ...
                        if (!settings.DisplayEnabled ||
                            !VideoDeviceKey.TryResolveOpenIndex(settings.DeviceKey, out var newIndex) ||
                            newIndex != index)
                        {
                            _logger.LogInformation(
                                ...
                                VideoDeviceKey.TryResolveOpenIndex(settings.DeviceKey, out var logged)
                                    ? logged
                                    : -1);
            ...
            while (!ct.IsCancellationRequested)
            {
                if (settingsAge.ElapsedMilliseconds >= SettingsRefreshMs)
                {
                    ...
                    var newFps = VideoFpsOptions.Normalize(
                        settings.TargetFps, VideoDeviceFpsCaps.PeekRates(settings.DeviceKey));
                    if (newFps != targetFps)
                    {
                        targetFps = newFps;
                        frameInterval = TimeSpan.FromSeconds(1.0 / targetFps);
        ...
        private VideoSettings ReadSettings()
        {
            try
            {
                // Memory snapshot only. GetAsync may re-read the JSON file
                // and must not run on this STA capture thread (sync-over-async
                // plus disk IO stole whole frame slots after 6812e07).
                return _settings.GetCached();
            }
            catch
            {
                return VideoSettings.Disabled;
            }
        }
        ...
        /// </summary>
        private static int EffectiveMaxWidth(VideoSettings settings, string? captureSize)
        {
            var configured = settings.MaxWidth < 0 ? 0 : Math.Clamp(settings.MaxWidth, 0, 1920);
        ...
        private static string DescribeOpenFailure(string? lastError)
        {
            if (string.IsNullOrWhiteSpace(lastError))
            {
                return OperatingSystem.IsMacOS()
                    ? "OpenCV returned IsOpened=false (device busy, index changed, or still releasing after a prior session). Retry in a few seconds; confirm Camera is allowed for Icom Web Control in System Settings."
                ...
                lastError.Contains("DllNotFound", StringComparison.OrdinalIgnoreCase))
            {
                    ...
                    RuntimeInformation.OSArchitecture == Architecture.X64)
                {
                    return "OpenCvSharp native library failed to load (missing Homebrew libavif on Intel Mac). " +
                           "Run: brew install libavif — then restart Icom Web Control.";
                }
            ...
            {
                return "Camera access denied. System Settings → Privacy & Security → Camera → enable Icom Web Control, then relaunch.";
            }
