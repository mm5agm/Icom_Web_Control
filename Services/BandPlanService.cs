...
namespace Icom_Web_Control.Services
{
    ...
    public sealed class BandPlanService : IBandPlanService
    {
        ...
        // FrequencyA/B setters call in here on every CI-V frequency update
        ...
        private string ResolveRegion()
        {
            ...
            try
            {
                ...
                //
                // Task.Run rather than a bare .GetAwaiter().GetResult(): IWC runs
                // a WinForms message loop (SystemTrayService), so a caller could
                // arrive on a thread carrying a WinForms SynchronizationContext.
                // Blocking there while GetSettingsAsync's file read tries to
                // resume on that same thread would deadlock. Task.Run moves the
                // continuation onto the thread pool, where it cannot.
