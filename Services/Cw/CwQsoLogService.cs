...
namespace Icom_Web_Control.Services.Cw
{
    ...
    /// Nothing is ever logged from the suggestion directly. Bench measurement
    /// on the Yaesu side had the decoder reporting full confidence on 592
    /// characters of junk, so a callsign picked out of the copy is a starting
    /// point for the operator to correct, never a fact.
    ///
    ...
    public sealed class CwQsoLogService
    {
        ...
        public static string LogPath => Path.Combine(
            ...
            "MM5AGM", "Icom Web Control", "iwc-log.adi");
        ...
        public async Task<CwQsoSaveResult> SaveAsync(CwQsoSave qso, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (!File.Exists(path))
                    text = AdifRecordWriter.Header("Icom Web Control",
