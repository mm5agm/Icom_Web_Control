 ...
 *
 * Ported from Yaesu Web Control's wwwroot/js/video/radio-display-ui.js and
 * kept line-for-line where it could be, so the two can be reconciled by
 * diff if the module ever moves to Radio_Web_Control_Core. What is gone is
 * Yaesu-only: the CAT scope-controls column (dock / undock / resize) and
 * the click-to-tune hotspot overlay. The IC-7300 MkII's HDMI mirror is
 * one-way and its scope is driven from the app's own spectrum panel.
 */
...
const CHANNEL_NAME = 'iwc-radio-display';
const AUTO_START_KEY = 'iwc.radioDisplayAutoStart';
...
function notifyPopoutReady() {
  ...
  try {
    if (window.opener && !window.opener.closed) {
      window.opener.postMessage({ type: 'iwc-radio-display-popout-ready' }, window.location.origin);
    }
  ...
  try {
    if (window.opener && !window.opener.closed) {
      window.opener.postMessage(
        { type: 'iwc-radio-display-reattach', stream: wantStream },
...
function bindChannel() {
  ...
  window.addEventListener('message', (ev) => {
    ...
    if (ev.data?.type === 'iwc-radio-display-reattach') {
      onReattachFromPopout(!!ev.data.stream);
    }
    if (ev.data?.type === 'iwc-radio-display-popout-ready') onPopoutReady();
  });
}
...
function bindControls() {
  ...
  document.getElementById('radioDisplayPopoutBtn')?.addEventListener('click', () => {
    ...
    const w = window.open('/RadioDisplay' + qs, 'iwc-radio-display', 'width=900,height=600');
    if (w) w.focus();
