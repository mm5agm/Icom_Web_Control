...
export class RadioDisplayPanel {
  ...
  constructor(imgId, containerId, badgeId, options = {}) {
    ...
    this._fitMode = localStorage.getItem('iwc.radioDisplayFit') || 'contain';
  ...
  show() {
    if (!this._ignoreHidePreference) {
      localStorage.setItem('iwc.radioDisplayVisible', '1');
    }
    const showRow = document.getElementById('radioDisplayShowRow');
    if (showRow) showRow.style.display = 'none';
  ...
  hide() {
    ...
    localStorage.setItem('iwc.radioDisplayVisible', '0');
  ...
  isHiddenByUser() {
    ...
    return localStorage.getItem('iwc.radioDisplayVisible') === '0';
  }

  setFitMode(mode) {
    this._fitMode = mode === 'cover' ? 'cover' : 'contain';
    localStorage.setItem('iwc.radioDisplayFit', this._fitMode);
    this._applyFit();
  }
  ...
  _sourceAspect() {
    ...
    return 16 / 9; // IC-7300 MkII HDMI is 1280×720; YWC's fallback is 4/3
  }
