    ...
    function createVfoVoiceControl(vfo, defaultStepHz) {
        ...
        if (stepSelect) {
            ...
            stepSelect.addEventListener('change', async function () {
                ...
                // number to the operator, so changing one changes both.
                // fromVoice stops tuning-step.js POSTing it straight back.
                window.iwcTuningStep?.set(vfo, stepHz, { fromVoice: true });
                try {
        ...
        // up the generated-from-config voice command list. Shared across both
        // mic buttons, so it lives in openVoiceHelp() outside this factory.
        ...
        conn.on('RadioStateUpdate', function (update) {
            ...
            controls.forEach(function (c) {
                if (update.property === 'VoiceNudgeStepHz' + c.vfo && c.stepSelect) {
                    ...
                    window.iwcTuningStep?.set(c.vfo, parseInt(update.value, 10), { fromVoice: true });
                }
            });
        });
    }
